...
typedef struct os_tcb {
    OS_STK          *OSTCBStkPtr;           /* Pointer to current top of stack                         指向当前栈顶指针*/
    ...
    INT32U           OSTCBStkSize;          /* Size of task stack (in number of stack elements)      	 任务堆栈长度*/
    INT16U           OSTCBOpt;              /* Task options as passed by OSTaskCreateExt()             创建任务时的选择项*/
    INT16U           OSTCBId;               /* Task ID (0..65535)                                      目前该域未被使用*/
#endif

    struct os_tcb   *OSTCBNext;             /* Pointer to next     TCB in the TCB list                 双链表后向搜索指针*/
    struct os_tcb   *OSTCBPrev;             /* Pointer to previous TCB in the TCB list                 双链表前向搜索指针*/
    ...
    INT32U           OSTCBDly;              /* Nbr ticks to delay task or, timeout waiting for event   任务等待的时限（节拍数）*/
    INT8U            OSTCBStat;             /* Task      status                                       任务当前状态标志（就绪，等信号 ，等消息，挂起，等互斥） */
    INT8U            OSTCBStatPend;         /* Task PEND status                                        */
    INT8U            OSTCBPrio;             /* Task priority (0 == highest)                            任务的优先级别*/

    INT8U            OSTCBX;                /* Bit position in group  corresponding to task priority   保存任务所在就绪表的就绪组里的位置（0~7）*/
    INT8U            OSTCBY;                /* Index into ready table corresponding to task priority   保存任务所在就绪表的就绪组号OSRdyGrp（0~7）*/
    OS_PRIO          OSTCBBitX;             /* Bit mask to access bit position in ready table          保存任务所在就绪表的就绪组里的Mask（1<<(0~7)）*/
    OS_PRIO          OSTCBBitY;             /* Bit mask to access bit position in ready group          保存任务所在就绪表的就绪组号Mask（1<<(0~7)）*/

#if OS_TASK_DEL_EN > 0u
    INT8U            OSTCBDelReq;           /* Indicates whether a task needs to delete itself         删除请求：为OS_TASK_DEL_REQ时代表其他任务发出删除请求*/
#endif
...
OS_EXT  INT32U            OSCtxSwCtr;               /* Counter of number of context switches           记录任务切换次数*/
...
OS_EXT  INT8U             OSLockNesting;            /* Multitasking lock nesting level                 调度器每上锁一次+1，解锁一次-1*/
...
OS_EXT  INT8U             OSTaskCtr;                       /* Number of tasks created                  创建任务总数*/
...
OS_EXT  OS_TCB           *OSTCBCur;                        /* Pointer to currently running TCB         当前运行任务控制块指针*/
OS_EXT  OS_TCB           *OSTCBFreeList;                   /* Pointer to list of free TCBs             指向初始空任务块链表的指针*/
OS_EXT  OS_TCB           *OSTCBHighRdy;                    /* Pointer to highest priority TCB R-to-R   指向当前就绪的最高优先级任务控制块*/
OS_EXT  OS_TCB           *OSTCBList;                       /* Pointer to doubly linked list of TCBs    指向TBC双链表的指针*/
OS_EXT  OS_TCB           *OSTCBPrioTbl[OS_LOWEST_PRIO + 1u];    /* Table of pointers to created TCBs   保存控制块指针的数组(方便检索)*/
OS_EXT  OS_TCB            OSTCBTbl[OS_MAX_TASKS + OS_N_SYS_TASKS];   /* Table of TCBs    任务控制块表              */
