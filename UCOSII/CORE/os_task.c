...
#if OS_TASK_SUSPEND_EN > 0u
INT8U  OSTaskSuspend (INT8U prio)/*任务挂起相当于把任务从就绪表里面注销*/
{
    BOOLEAN    self;
    OS_TCB    *ptcb;
    INT8U      y;
...
#endif
    OS_ENTER_CRITICAL();
    if (prio == OS_PRIO_SELF) {                                 /* See if suspend SELF                 判断是否挂起自身*/
        prio = OSTCBCur->OSTCBPrio;
        self = OS_TRUE;
    } else if (prio == OSTCBCur->OSTCBPrio) {                   /* See if suspending self              判断是否挂起自身*/
        self = OS_TRUE;
    } else {
