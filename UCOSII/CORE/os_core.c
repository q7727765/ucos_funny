...
void  OS_Sched (void)
{
    ...
    OS_ENTER_CRITICAL();
    if (OSIntNesting == 0u) {                          /* Schedule only if all ISRs done and ...      判断调度器是否上锁 */
                   ...
                   INT16U   opt)
{
    ...
    if (ptcb != (OS_TCB *)0) {
        ...
        OS_EXIT_CRITICAL();
        ptcb->OSTCBStkPtr        = ptos;                   /* Load Stack pointer in TCB                初始化栈顶指针*/
        ptcb->OSTCBPrio          = prio;                   /* Load task priority into TCB              初始化优先级*/
        ptcb->OSTCBStat          = OS_STAT_RDY;            /* Task is ready to run                     初始化任务状态*/
        ptcb->OSTCBStatPend      = OS_STAT_PEND_OK;        /* Clear pend status                        */
        ptcb->OSTCBDly           = 0u;                     /* Task is not delayed                      初始化任务等待时限*/
        ...
        OS_ENTER_CRITICAL();
        OSTCBPrioTbl[prio] = ptcb;												/*任务登记在优先级控制块检索表中*/
